...
class ConfigStatusResponseMapping:
    ...
    def get_status_values(self, raw_response: list[int]) -> dict[str, int]:
        response = {
            ...
            if k != "starts_with" and v in range(len(raw_response))
        }
...
class ConfigSetting(JsonSerializable):
    ...
    def get_kwargs(self) -> dict[str, Any]:
        return {
            k: v
            for k, v in self.__dict__.items()
            if k not in ["name", "type", "default_value", "update_sequence"]
        }
...
class DeviceConfiguration:
    ...
    def __init__(self, raw_configuration: dict[str, Any]):
        ...
        self.command_interface_index = raw_config.get(
            "command_interface_index", (-1, -1)
        )
        self.command_report_id = raw_config.get("command_report_id", 0x00)
        self.listen_interface_indexes = raw_config.get("listen_interface_indexes", [])

        online_status = raw_config.get("online_status", None)
        self.online_status = (
            OnlineStatusConfig(**online_status) if online_status else None
        )

        if not self.name:
            raise ValueError(
                "Invalid configuration: 'device.name' must be specified and non-empty"
            )
        if self.vendor_id == 0:
            raise ValueError(
                "Invalid configuration: 'device.vendor_id' must be specified and non-zero"
            )
        if not self.product_ids:
            raise ValueError(
                "Invalid configuration: 'device.product_ids' must be a non-empty list"
            )
        if not self.listen_interface_indexes:
            raise ValueError(
                "Invalid configuration: 'device.listen_interface_indexes' must be a non-empty list"
            )
        if any(i < 0 for i in self.listen_interface_indexes):
            raise ValueError(
                "Invalid configuration: 'device.listen_interface_indexes' must contain only non-negative integers"
            )
        ...
        if raw_padding:
            ...
            raise ValueError(
                "Invalid configuration: 'device.command_padding' must be specified"
            )
        ...
        for setting_section, settings in raw_settings.items():
            ...
            for setting_name, setting_values in settings.items():
                ...
                self.settings[setting_section].append(
                    ConfigSetting(
                        name=setting_name,
                        type=setting_type,
...
def load_device_configurations() -> list[DeviceConfiguration]:
    ...
    for config_path in DEVICES_CONFIG_FOLDER:
        ...
        for file in config_path.glob("*.yaml"):
            ...
            logger.info(
                f"Found: {config.name} (0x{config.vendor_id:04x}, {[f'0x{pid:04x}' for pid in config.product_ids]}) from {file}"
            )
...
def parsed_status(
    raw_status: dict[str, int] | None, device_config: DeviceConfiguration | None
) -> dict[str, Any]:
    ...
    for key, raw_value in raw_status.items():
        status_parse_config = next(
            (csp for sp, csp in device_config.status_parse.items() if sp == key), None
        )
        ...
        parser = next(
            (
                p
                for p in status_parsers
                if getattr(p, "_status_type") == status_parse_config.type.value
            ),
            None,
        )
        if parser is None:
